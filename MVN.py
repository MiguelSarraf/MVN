...
class MVN:
	...
	def step(self):
		...
		self.OP.set_value(self.ula.execute(7, self.IR.get_value(), 0x1000))
		self.OI.set_value(self.ula.execute(7, self.ula.execute(6, self.IR.get_value(), 0x0010), 0x0010))
		try:
			res=self.ula.execute(self.OP.get_value(), self.AC.get_value(), self.OI.get_value())
			if self.OP.get_value() in [1,2]:
				if res:
					self.IC.set_value(self.OI.get_value())
				else:
					self.IC.set_value(self.IC.get_value()+2)
			else:
				self.AC.set_value(res)
				self.IC.set_value(self.IC.get_value()+2)
		except:
	...
	def ld(self):
		self.AC.set_value(self.mem.get_value(self.OI.get_value()))
		self.IC.set_value(self.IC.get_value()+2)
		return True
	...
	def rs(self):
		self.IC.set_value(self.mem.get_value(self.mem.get_value(self.OI.get_value())))
		return True
	...
	def print_state(self):
		print(hex(self.MAR.get_value())[2:].zfill(4)+" "+hex(self.MDR.get_value())[2:].zfill(4)+" "+hex(self.IC.get_value())[2:].zfill(4)+" "+hex(self.IR.get_value())[2:].zfill(4)+" "+hex(self.OP.get_value())[2:].zfill(4)+" "+hex(self.OI.get_value())[2:].zfill(4)+" "+hex(self.AC.get_value())[2:].zfill(4))

	def set_memory(self, file):
		#TODO
		pass